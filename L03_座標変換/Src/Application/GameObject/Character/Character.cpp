...
void Character::DrawSprite()
{
	...
	if (_camera)
	{
		Math::Vector3 circlePos{};
		_camera->ConvertWorldToScreenDetail(m_mWorld.Translation(),circlePos);
		KdShaderManager::Instance().m_spriteShader.DrawCircle(circlePos.x, circlePos.y, 10, &kRedColor);
	}
}
